...
/// Optimized for delivery/packing sheets
class SuspendedSheet2Screen extends StatefulWidget {
  const SuspendedSheet2Screen({super.key});

  @override
  State<SuspendedSheet2Screen> createState() => _SuspendedSheet2ScreenState();
}

class _SuspendedSheet2ScreenState extends State<SuspendedSheet2Screen> {
  final ApiService _apiService = ApiService();
  final TextEditingController _searchController = TextEditingController();

  List<SuspendedSheet2Sale> _sales = [];
  List<SuspendedSheet2Sale> _filteredSales = [];
  ...
  Future<void> _loadData() async {
    ...
    final response = await _apiService.getSuspendedSheet2(locationId: locationId);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Delivery Sheet', style: TextStyle(fontSize: 18)),
        actions: [
          // Location selector
                  ...
                  : _filteredSales.isEmpty
                      ? Center(
                          child: Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              Icon(Icons.local_shipping_outlined, size: 64, color: isDark ? Colors.grey.shade600 : Colors.grey.shade400),
                              const SizedBox(height: 16),
                              Text(
  ...
  Widget _buildSaleCard(SuspendedSheet2Sale sale, int number, bool isDark) {
    return Card(
      ...
      child: Column(
        ...
        children: [
          ...
          // Items table (no prices, includes Free column)
          Padding(
            padding: const EdgeInsets.all(12),
            child: Column(
              children: [
          ...
          // Footer with Print and PDF buttons
          Container(
  ...
  Future<void> _exportPdf(SuspendedSheet2Sale sale) async {
    try {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  Future<void> _downloadPdf(SuspendedSheet2Sale sale) async {
    try {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
